...
fn main() {
    ...
    let mut total_50 = 0;
    for ((i_1, g_1), (i_2, g_2)) in possible_cheats.into_iter() {
    ...
    let mut cheats: Vec<(usize, usize)> = cheats.into_iter().collect();
    cheats.sort_by_key(|(key, _)| *key);

    for (key, val) in &cheats {
        if *key >= 50 {
            println!("There are {val} cheats that save {key}");
        }
    }
    println!(":3");
    for (key, val) in &cheats {
    ...
    println!("Total cheats >100: {over_100}");
    println!("Total cheats >100: {total_50}");
}
